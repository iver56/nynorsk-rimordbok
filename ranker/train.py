...
class Vectorizer(object):
    METADATA_FILE_PATH = os.path.join(DATA_DIR / "ranker_model" / "metadata.json")
    MODEL_FILE_PATH = os.path.join(DATA_DIR / "ranker_model" / "model.pkl")
    ...
    def __init__(self, mode="training"):
        ...
        if mode == "training":
            ...
            first_rhyme_candidates = [
                self.preprocess_word(example["first_rhyme_candidate"])
                for example in dataset
            ]
            second_rhyme_candidates = [
                self.preprocess_word(example["second_rhyme_candidate"])
                for example in dataset
            ]

            characters = (
                set("".join(search_words))
                .union(set("".join(first_rhyme_candidates)))
                .union(set("".join(second_rhyme_candidates)))
            )
    ...
    def train_model(self, dataset):
        input_vectors = [self.vectorize_triplet(example) for example in dataset]
        target_vectors = [example["rank"] for example in dataset]
    ...
    def vectorize_triplet(self, rhyme_ranking_object):
        search_word_vector = self.vectorize_string(rhyme_ranking_object["search_word"])
        first_rhyme_candidate_vector = self.vectorize_string(
            rhyme_ranking_object["first_rhyme_candidate"]
        )
        second_rhyme_candidate_vector = self.vectorize_string(
            rhyme_ranking_object["second_rhyme_candidate"]
        )

        input_vector = np.array(
            search_word_vector
            + first_rhyme_candidate_vector
            + second_rhyme_candidate_vector
        )
        return input_vector
